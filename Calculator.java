...
public class Calculator {
    ...
    public void updateCalc(LinkedList<Song> songs) {
        
        reader.updateResponse(songs);
        hobbProp = new LinkedList<int[][]>();
        regionProp = new LinkedList<int[][]>();
        majorProp = new LinkedList<int[][]>();
...
// ----------------------------------------------------------------------------
    ...
    private void notSEorNE() {
        ...
        for (int i = 0; i < songs.getSize(); i++) {
            for (int j = 0; j < students.getSize(); j++) {
                if (students.get(j)
                    .getRegion() == Region.UNITED_STATES_OTHER_THAN_SOUTHEAST_OR_NORTHWEST) {
                    if (students.get(j).getResponse(i, 1) == Response.YES) {
                        heardCount++;
                    }
                    if (students.get(j).getResponse(i, 0) == Response.YES) {
            ...
            if (totalCount != 0) {
                regionProp.get(i)[3][0] = heardCount * 100 / totalCount;
                regionProp.get(i)[3][1] = likesCount * 100 / totalCount;
            }
            else {
