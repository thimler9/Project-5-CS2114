...
public class GUI {
    ...
    private int songIndex; // Keeps track of which 5 songs for the next and
    //prev methods
...
// ----------------------------------------------------------------------------
    ...
    public void addLegend() {
        ...
        Shape legend = new Shape(950, 340, 200, 230);
        ...
        for (int i = 0; i < max; i++) {
            if (i < 3) {
                songTitle = new TextShape(203 + 350 * i - songs.get(songIndex
                    * 5 + i).getSong().length() * 3, 25, 
                    songs.get(songIndex * 5 + i).getSong());
                ...
                songTitle = new TextShape(203 + 350 * (i - 3) - songs.get(
                    songIndex * 5 + i).getSong().length() * 3, 325,
                    songs.get(songIndex * 5 + i).getSong());
                songArtist = new TextShape(203 + 350 * (i - 3) - songs.get(
                    songIndex * 5 + i).getArtist().length() * 3, 350, 
                    songs.get(songIndex * 5 + i).getArtist());
                songTitle.setBackgroundColor(Color.WHITE);
                songArtist.setBackgroundColor(Color.WHITE);
                window.addShape(songTitle);
                window.addShape(songArtist);
...
// ----------------------------------------------------------------------------
     ...
     * 
     * @param enumeration
     *            the type of enum that is currently being displayed
     */
    ...
    public void updateLegend(String string) {
        if (string.equals("Region")) {
            ...
            legendTitle = new TextShape(1000, 325, "Region Legend");
        }
        if (string.equals("Hobby")) {
            ...
            legendTitle = new TextShape(1000, 325, "Hobby Legend");
        }
        if (string.equals("Major")) {
            ...
            legendTitle = new TextShape(1000, 325, "Major Legend");
        }
    ...
    public void updatePercents(String string) {
        ...
        for (int i = 0; i < max; i++) {
            ...
            if (i < 3) {
                ...
                secondHeardPerc = new Shape(201 
                    - enumList.get(index)[1][0] + (i
                    - 3) * 350, 425, enumList.get(index)[1][0], 50);
                thirdHeardPerc = new Shape(201 - enumList.get(index)[2][0] + (i
                    - 3) * 350, 475, enumList.get(index)[2][0], 50);
                fourthHeardPerc = new Shape(201 
                    - enumList.get(index)[3][0] + (i
